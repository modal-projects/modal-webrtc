    ...
    async negotiate() {
        try {
            ...
            this.dataChannel.onopen = (event) => {
                ...
                this.dispatchEvent(new CustomEvent('ondatachannel', { 
                    detail: "channel connected"
                }));
            };
            this.dataChannel.onmessage = (event) => {
                // event.data is a UTF-8 encoded bytestring (ArrayBuffer)
                const text = new TextDecoder().decode(event.data);
                let data;
                try {
                    data = JSON.parse(text);
                } catch (e) {
                    console.error('Failed to parse datachannel message as JSON:', text, e);
                    return;
                }
                console.log('Datachannel message received (JSON):', data);
                this.dispatchEvent(new CustomEvent('onMessage', { 
                    detail: { data }
                }));
            };
