...
function updateStatusDisplay() {
    ...
    // Update real-time transcript in light green
    statusDisplay.innerHTML += `<div class="status-line" style="color: #008000;">${realtimeTranscript}</div>`;
...
webrtcClient.addEventListener('onMessage', (event) => {
    // console.log("Received message:", event.detail.data);
    if (event.detail.data.type === 'final_transcript') {
        console.log("Final transcript:", event.detail.data.text);
        statusHistory.push(event.detail.data.text);
        realtimeTranscript = "";
    } else if (event.detail.data.type === 'realtime_transcript') {
        // console.log("Real-time transcript:", event.detail.data.text);
        realtimeTranscript = event.detail.data.text;
        
    }
    updateStatusDisplay();
});

webrtcClient.addEventListener('ondatachannel', (event) => {
...
async function handleStartWebcam() {
    try {
        await webrtcClient.startWebcam(true);
